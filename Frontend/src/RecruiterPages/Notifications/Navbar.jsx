...
import ProfileGreen from './profileGreen';

const Navbar = ({ pageName }) => {
  const [searchQuery, setSearchQuery] = useState('');
  const [isMobile, setIsMobile] = useState(window.innerWidth < 768); // Check if screen width is mobile size
  const [showSearchInput, setShowSearchInput] = useState(false); // Toggle for search input on mobile
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e) => {
      if (searchRef.current && !searchRef.current.contains(e.target)) {
        setShowSearchInput(false); // Close search bar if clicked outside
      }
    };
  ...
  return (
    <nav className=" w-full m-0 flex items-center text-white py-2">
  ...
  <>
    {showSearchInput ? (
      <form
        ref={searchRef} // Reference the form to detect clicks outside
        onSubmit={handleSearchSubmit}
        className="absolute top-full left-0 w-11/12 p-2 border-2 border-gray-300 rounded-full bg-white mx-4 mt-2 " // Added mr-4
      >
        <div className="flex items-center">
          <FaSearch className="text-[#5F9D08] mr-3 text-xl" /> {/* Icon with some margin */}
          <input
            type="text"
            value={searchQuery}
      ...
      <button onClick={toggleSearchInput} className="ml-auto mr-4 flex items-center">
        <FaSearch className="text-[#5F9D08] text-2xl " /> {/* Icon with padding on top */}
      </button>
    )}
  </>
) : (
  ...
  >
    <FaSearch className="text-[#5F9D08] mr-3 text-xl" /> {/* Icon with margin */}
    <input
      type="text"
      value={searchQuery}
      onChange={handleSearchChange}
